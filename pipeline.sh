...
if [ -f "ingestion/tv/ingest_tv_rss.py" ]; then
  echo "=== Étape 1 : Ingestion TV RSS ==="
  $PYTHON ingestion/tv/ingest_tv_rss.py
else
  echo "⚠️ ingestion/tv/ingest_tv_rss.py introuvable, étape ingestion TV ignorée."
fi

# 4.2 NLP : nettoyage + lemmatisation + entités
if [ -f "processing/nlp/process_articles.py" ]; then
  echo "=== Étape 2 : NLP (Stanza + spaCy) ==="
  $PYTHON processing/nlp/process_articles.py
else
  echo "⚠️ processing/nlp/process_articles.py introuvable, étape NLP ignorée."
fi

# 4.3 Extraction des top 10 mots (keywords_daily)
if [ -f "processing/keywords/extract_keywords.py" ]; then
  echo "=== Étape 3 : Extraction des mots-clés ==="
...
# 4.4 Topic modeling (topics_daily)
if [ -f "processing/topics/extract_topics.py" ]; then
  echo "=== Étape 4 : Extraction des sujets (topics) ==="
